...
public class VoiceFragment extends Fragment {
    ...
    private void initializeViews(View view) {
        ...
        LinearLayout layoutUnderstood = view.findViewById(R.id.layout_understood);
    ...
    private void setupMicButton() {
        btnMic.setOnClickListener(v -> simulateVoice());
    }

    private void simulateVoice() {
        ...
        String example = "Ramesh ne 2 kg dal liya 120 rupaye";
        int[] index = {0};

        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                if (index[0] < example.length()) {
                    voiceText = example.substring(0, index[0] + 1);
                    updateUI();
                    index[0]++;
                    ...
                    updateUI();
                    handler.postDelayed(() -> {
                        parsedResult = VoiceParser.parse(example, getCustomerList());
                        status = "done";
                        stopPulseAnimation();
                        isListening = false;
                        updateUI();
    ...
    private void updateUI() {
        ...
        LinearLayout layoutUnderstood = getView().findViewById(R.id.layout_understood);
